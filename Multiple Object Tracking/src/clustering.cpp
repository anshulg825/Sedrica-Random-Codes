...
bool label_comparison(SLRPointXYZIRL a , SLRPointXYZIRL b){
...
void ScanLineRun::velodyne_callback_(const sensor_msgs::PointCloud2ConstPtr& in_cloud_msg){
        ...
        if(!runs_[i].empty()){
            ...
            // if(dummycloud.points.size()<100){
                for(int ui=0;ui<dummycloud.points.size();ui++){
                    if(dummycloud.points[ui].x<20&&dummycloud.points[ui].x>-20) {laserCloud->points.push_back(dummycloud.points[ui]); to++;}

                    //std::cout<<"life sucks so does my code  "<<dummycloud.points[ui].z<<endl;
                }
            // }
                if(dummycloud.points.size()>0){
                pcl::PointCloud<pcl::PointXYZ>::Ptr r (new pcl::PointCloud<pcl::PointXYZ>);
               for(int ko=0;ko<dummycloud.points.size();ko++){
                    if(dummycloud.points[ko].x<20&&dummycloud.points[ko].x>-20){
                        pcl::PointXYZ rtt;
                        rtt.z=dummycloud.points[ko].z;
                        rtt.x=dummycloud.points[ko].x;
                        rtt.y=dummycloud.points[ko].y;
                        r->points.push_back(rtt);
                    }
                }
                std::cout<<"To the infinity and beyond "<<r->points.size()<<endl;
                
                pcl::PointCloud<pcl::Normal>::Ptr normals(new pcl::PointCloud<pcl::Normal>);
                pcl::PointCloud<pcl::SHOT352>::Ptr descriptors(new pcl::PointCloud<pcl::SHOT352>());
                pcl::NormalEstimation<pcl::PointXYZ, pcl::Normal> normalEstimation;
                normalEstimation.setInputCloud(r);
                ...
                std::cout<<"I have failed the city  " <<descriptors->size()<<"GGG "<<r->points.size()<<endl;
                //std::cout<<"Valar Morghulis "<<descriptors->points[1]<<endl;
            }
            ...
            if(vectorcloud->points.size()>100){
   ...
   // clusters->clear();
    ...
    sort(laserCloud->points.begin(), laserCloud->points.end(),label_comparison);
    pcl::PointCloud<SLRPointXYZIRL>::iterator top = laserCloud->points.end();
    top = top-1;
    std::cout<<"Did you miss me "<<top->label<<"    Yes my friend, Happiness";
