...
visualization_msgs::Marker mark_cluster(pcl::PointCloud<ClusterpointcloudXYZ>::Ptr cloud_cluster, 
...
void ScanLineRun::update_labels_(int scan_line){
    ...
    for(int j_idx=0;j_idx<point_size_j_idx;j_idx++){
        ...
        for(int l=scan_line-1;l>=0;l--){
            ...
            if(fpfh.dist()<threshold) {
                pcl_conversions.push_back(target);
                int t = point_cloud.size();
                p.label= current.label;
                idt = runs_/t*p_dummy.label;
                merge_labels(p.label, p_dummy.label, idt);
                th_run_.scan(target);
        ...
        if(!runs_[i].empty()){
            ...
            // if(dummycloud.points.size()<100){
                for(int ui=0;ui<dummycloud.points.size();ui++){
                    if(dummycloud.points[ui].x<20&&dummycloud.points[ui].x>-20) laserCloud->points.push_back(dummycloud.points[ui]);
            ...
            ma.markers.push_back(mark_cluster(vectorcloud,"box", cnt, 1.0f,2.0f, 1.0f));}
            std::cout<<ma.markers.size()<<endl;
            vectorcloud->clear();
